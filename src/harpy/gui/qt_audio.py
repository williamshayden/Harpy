...
from harpy.gui.controller import (
    AudioCommand as LegacyAudioCommand,
)
from harpy.gui.controller import (
    AudioCommandKind as LegacyAudioCommandKind,
)
...
class SynthAudioDevice(QIODevice):
    def __init__(
        ...
        self._commands: queue.SimpleQueue[LegacyAudioCommand] = queue.SimpleQueue()
        self._staging = bytearray()
        self.open(QIODevice.OpenModeFlag.ReadOnly)

    def submit(self, command: LegacyAudioCommand) -> None:
        self._commands.put(command)
    ...
    def _drain_commands(self) -> None:
        while True:
            try:
                ...
                return
            if command.kind is LegacyAudioCommandKind.NOTE_ON:
                if command.pitch is None:
                    raise RuntimeError("validated NOTE_ON command lost its pitch")
                self._voice.note_on(command.pitch)
            elif command.kind is LegacyAudioCommandKind.NOTE_OFF:
                self._voice.note_off()
            elif command.kind is LegacyAudioCommandKind.RESET:
                self._voice.reset()
                self._staging.clear()
                self._sample_history.clear()
