...
It adds numeric ADSR authoring, one constrained quadratic Bézier curve for each shaped
stage, strict patch-schema version 2, a compact envelope inspector, and a professional
...
- exact numeric editing of Attack, Decay, Sustain, and Release;
...
- `EnvelopeEditor` owns numeric fields, draft presentation, validation feedback, and
  complete-patch commit signals.
- `EnvelopeGraph` paints the preview and owns handle interactions. It emits proposed
  normalized curvature values; it does not own patch truth.
...
- Attack, Decay, and Release values with explicit time units;
- Sustain with a `dB` unit;
- one control handle for Attack, Decay, and Release;
- the selected handle's precise curvature value;
- `Reset curves to linear`;
- patch status: `Active`, `Editing`, or `Pending`;
- compact Sine and output-gain facts;
- Load and Save As actions.

Numeric fields are the timing truth. Time fields accept an explicit `ms` or `s` suffix;
a bare value uses the suffix currently shown by that field. Canonical display uses
milliseconds below one second and seconds at or above one second. They use the same
exact-value caching principle as the frequency entry: programmatic updates do not emit,
valid typed values survive display rounding, unchanged commits retain their exact value,
invalid commits preserve text and show one field-specific error, and Escape restores the
latest valid authored value.
...
curve. Handle position is a projection of the model value, using
`c = 2 * (P1.y - (a + b) / 2) / (b - a)` whenever the stage has a nonzero amplitude
span; the graph never stores a second copy of curvature. The reset action sets all three
values to zero in one commit.
...
visually, the precise numeric and keyboard curvature editors remain authoritative.
...
- Stage boundaries and numeric duration labels make the transform clear.
...
- All four numeric fields have explicit accessible names and unit descriptions.
...
- Reset, Load, and Save As retain non-Space keyboard routes.
...
│                          │                           │                 │
│ physical-time capture    │ log-frequency capture     │ graph + fields  │
│                          │                           │ patch actions   │
└──────────────────────────┴───────────────────────────┴─────────────────┘
```
...
- Syntactically incomplete field text remains local and does not flash global errors on
  every keystroke.
- A failed commit identifies the exact field beside the editor and uses the existing
  single error surface for a concise full explanation.
...
- Numeric fields preserve exact values across display rounding and programmatic updates.
- Numeric and graph edits create identical immutable patches.
- Drag emits drafts but only one complete commit on release.
- Handle bounds, reset behavior, keyboard stepping, focus, and accessibility are real
  widget-event tests.
- Loading v1/v2 refreshes all fields and handles without feedback loops.
- Active, Editing, Pending, file-error, and device-error states render truthfully.
